...
> is that **the binding passes 4724 golden cases**, and that is a surface check
...
an ES module a browser simply reads (435KB gzipped, 1567KB before compression).
...
It passes **4724 golden cases** — every one in the table but eleven. Those eleven
...
covers 4672 cases, and the 53 *it* does not see are this side's alone (1-D and 3-D
...
borch.ts itself has written TS bodies for 4048 cases. **The remaining 687 are two
things**: 687 deliberately not carried across, and 0 owed. The binding
...
two, the table read 9.9 / 18.8 ms on Apple and 6.7 / 14.5 ms on the 4090. The third
was the convolution kernel — not its arithmetic but its grid: 512 → 512 channels on a
...
And **4735 golden cases** compare all three implementations against **the same
expected values.** The core covers 4672 cases, leaving out the 53 that are
