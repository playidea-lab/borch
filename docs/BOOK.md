...
> is that **the binding passes 4753 golden cases**, and that is a surface check
...
an ES module a browser simply reads (186KB gzipped, 663KB before compression).
...
It passes **4753 golden cases** — every one in the table but eleven. Those eleven
...
covers 4701 cases, and the 53 *it* does not see are this side's alone (1-D and 3-D
...
borch.ts itself has written TS bodies for 4057 cases. **The remaining 707 are two
things**: 687 deliberately not carried across, and 20 owed. The binding
...
And **4764 golden cases** compare all three implementations against **the same
expected values.** The core covers 4701 cases, leaving out the 53 that are
