...
> **아직 안 본 것 — CORS.** fetch 경로는 **로컬 서버 주소**로만 확인했다. 진짜 CIFAR-10
> 원본(`cs.toronto.edu`)이 브라우저 요청에 CORS 헤더를 주는지는 확인하지 않았고,
> 안 주면 브라우저에서 직접 못 받는다. 그때는 CORS 를 주는 미러를 쓰거나 사용자가
> 파일을 골라 넣는 길(File 입력)을 열어야 한다. **주소를 인자로 둔 이유가 이것이다.**
