...
> 커밋되는 것은 sha256 여덟 줄(`tests/browser/assets.lock`)뿐이다.
...
| | 라이선스 | 확인 |
|---|---|---|
| **TensorFlow.js** (`tfjs`, `tfjs-backend-webgpu`) | Apache-2.0 · Copyright 2024 Google LLC | `tf.min.js` 머리 주석 |
| **Pyodide** | **MPL-2.0** | pyodide/pyodide 0.27.2 의 `LICENSE` |
| **CPython 표준 라이브러리** | PSF License | `python_stdlib.zip` 에 동봉 |
| **numpy** | BSD-3-Clause | 위와 같음 |

---

## 지켜야 하는 것

### Apache-2.0 (TF.js) — 표시를 남긴다

재배포할 때 라이선스 사본과 저작권 표시를 함께 둔다. 우리 것도 Apache-2.0 이라
충돌하지 않는다.
