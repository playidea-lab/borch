...
5,314 줄짜리 결속이고, 그 차이(1.5분 대 1.6분)가 Pyodide 를 한 번 지나는 값이다.
...
같은 것을 준다. `borch_webgpu` 도 같은 모양이고 5,314 줄에 여섯 파일이다.
...
> 닿지 않는다. 그쪽에 대해 말할 수 있는 것은 **골든 2263건이 지난다**는 것뿐이고
> (표 전체는 2288 건인데, 복소수 25 건은 코어에만 있다),
...
그리고 **골든 2288건**이 세 구현을 **같은 기대값**에 대조한다. 셋이 보는 수는 다르다 —
범위가 **양쪽으로** 갈리기 때문이다. 코어는 브라우저 전용(1·3 차원 합성곱처럼 코어가
일부러 거절하는 것) 53 건을 빼고 2235 건을 보고, 브라우저 쪽은 코어에만 있는 복소수
25 건을 빼고 2263 건을 본다. 없는 것을 물으면 그건 검사가 아니라 오답이다.
진짜 torch 를 브라우저에 넣을 수 없어서, 네이티브에서 기대값을 굳혀 브라우저로
들고 간다.
