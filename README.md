...
> is that **3072 golden cases pass**, and that is a surface check rather than a
...
| what is here | **22 of the 41 names `torchvision.transforms` carries.** Composition — `Compose`, `Lambda`, `RandomApply`, `RandomChoice`, `RandomOrder`. Tensors — `ToTensor`, `Normalize`, `LinearTransformation`. Geometry — `Resize`, `CenterCrop`, `RandomCrop`, `RandomResizedCrop`, `FiveCrop`, `TenCrop`, `Pad`, `InterpolationMode`. Augmentation — `RandomHorizontalFlip`, `RandomVerticalFlip`, `Grayscale`, `RandomGrayscale`, `RandomErasing`, `ColorJitter`. Plus `augment_batch`, which torchvision does not have. **`transforms.functional` holds 23 of the 37 names `torchvision.transforms.functional` carries** — `crop`, `center_crop`, `resized_crop`, `five_crop`, `ten_crop`, `pad`, `resize`, `hflip`, `vflip`, `rgb_to_grayscale`, `to_grayscale`, `normalize`, `to_tensor`, `erase`, `get_dimensions`, `get_image_size`, `get_image_num_channels`, `InterpolationMode`, and the photometric `adjust_brightness`, `adjust_contrast`, `adjust_saturation`, `adjust_hue`, `adjust_gamma` — so `import borchvision.transforms.functional as F` is a line that runs. **What is absent carries a reason** in `tests/torch_gap.py`, and what carries none is the to-do list |
|---|---|
...
It passes **all 3072 golden cases** — nothing in the table is skipped on this side
alone. The core covers 3019 cases, and the remaining 53 are ones the core refuses
...
borch.ts itself has written TS bodies for 2655 cases. The remaining 417 are **two different
...
twice. The other 57 are **owed**: the `borchvision` transforms that arrived after
borch.ts's `vision.ts` was written, which has seven of the twenty-one. Those are a
backlog rather than a decision, and calling all 417 deliberate would make the
...
> against the real 2655, because the names are built programmatically.
...
And **3072 golden cases** compare all three implementations against **the same
expected values.** The core covers 3019 cases, leaving out the 53 that are
