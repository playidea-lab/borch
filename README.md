...
5,647 줄짜리 결속이고, 그 차이(1.5분 대 1.6분)가 Pyodide 를 한 번 지나는 값이다.
...
같은 것을 준다. `borch_webgpu` 도 같은 모양이고 5,647 줄에 여섯 파일이다.
...
> 닿지 않는다. 그쪽에 대해 말할 수 있는 것은 **골든 2297건이 지난다**는 것뿐이고
> (표 전체는 2358 건인데, 푸리에 변환 61 건은 아직 코어에만 있다),
...
| **푸리에** | `fft.fft`/`ifft`/`rfft`/`irfft` · `fftfreq`/`rfftfreq`/`fftshift`/`ifftshift` · `stft`/`istft` — `n`·`dim`·`norm` 과 역전파. **아직 코어(numpy)에만** |
...
골든 **2297 건**을 지난다. 표 전체는 2358 건인데 **푸리에 변환 61 건이 여기만
빠진다** — 코어에는 있고 borch.ts 에 아직 없다(반대로 1·3 차원 합성곱 53 건은 코어가
일부러 거절하는 것이라 코어가 건너뛴다).

borch.ts 자신은 1804 건에 TS 본문을 써 두었다. 나머지 554 건은 **일부러 안 옮겼다** —
...
그리고 **골든 2358건**이 세 구현을 **같은 기대값**에 대조한다. 코어는 브라우저
전용(1·3 차원 합성곱처럼 코어가 일부러 거절하는 것) 53 건을 빼고 2305 건을 보고,
브라우저 쪽은 코어에만 있는 푸리에 변환 61 건을 빼고 2297 건을 본다 —
