...
2,827 줄짜리 결속이고, 그 차이(1.5분 대 1.6분)가 Pyodide 를 한 번 지나는 값이다.
...
같은 것을 준다. `borch_webgpu` 도 같은 모양이고 2,827 줄에 여섯 파일이다.
...
> 닿지 않는다. 그쪽에 대해 말할 수 있는 것은 **골든 1001건이 지난다**는 것뿐이고,
...
같은 골든 **1001 건 전부**를 지난다 — borch.ts 자신과 같은 수다. 코어(numpy)는 그중
948 건을 보는데, 나머지 53 건은 코어가 일부러 거절하는 것들(1·3 차원 합성곱,
랭크 7·8)이라 안 묻는다.
...
그리고 **골든 1001건**이 세 구현을 **같은 기대값**에 대조한다. (코어는 브라우저 전용
케이스를 건너뛰어 948건을 본다 — 코어가 일부러 거절하는 것을 코어에게 물으면 그건
