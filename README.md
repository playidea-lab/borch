...
> is that **3490 golden cases pass**, and that is a surface check rather than a
...
It passes **3490 golden cases** — every one in the table but five. Those five are
the core's alone: complex eigenvalues, and there is no complex dtype on this side.
The core covers 3437 cases, and the 53 *it* does not see are this side's alone
...
borch.ts itself has written TS bodies for 3081 cases. **The remaining 409 are two
things**: 409 deliberately not carried across, and 0 owed. The binding
...
And **3490 golden cases** compare all three implementations against **the same
expected values.** The core covers 3437 cases, leaving out the 53 that are
...
rasteriser, and for two days every browser golden here ran on it — the same 3437
