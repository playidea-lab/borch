...
6,284 줄짜리 결속이고, 그 차이(1.5분 대 1.6분)가 Pyodide 를 한 번 지나는 값이다.
...
| `_tensor.py` | 1,236 | `Tensor` 와 autograd |
| `_ops.py` | 7,894 | 수학 · 모양 · `nn.functional` |
| `_nn.py` | 2,866 | 층 · 순환 · 트랜스포머 |
...
같은 것을 준다. `borch_webgpu` 도 같은 모양이고 6,284 줄에 여섯 파일이다.
...
> 닿지 않는다. 그쪽에 대해 말할 수 있는 것은 **골든 2775건이 지난다**는 것뿐이고,
...
**목록에 들어오려면 틀렸던 케이스를 이름으로 댈 수 있어야 한다.** 지금 여섯이 다
...
**골든 2775 건 전부**를 지난다 — 표에서 이쪽만 건너뛰는 것이 하나도 없다. 코어는
그중 2722 건을 보는데, 나머지 53 건은 코어가 일부러 거절하는 것들(1·3 차원 합성곱,
랭크 7·8)이라 안 묻는다.

borch.ts 자신은 2086 건에 TS 본문을 써 두었다. 나머지 676 건은 **일부러 안 옮겼다** —
...
그리고 **골든 2775건**이 세 구현을 **같은 기대값**에 대조한다. 코어는 브라우저
전용(1·3 차원 합성곱처럼 코어가 일부러 거절하는 것) 53 건을 빼고 2709 건을 본다 —
