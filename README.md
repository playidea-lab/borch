...
> is that **3293 golden cases pass**, and that is a surface check rather than a
...
It passes **3293 golden cases** — every one in the table but five. Those five are
the core's alone: complex eigenvalues, and there is no complex dtype on this side.
The core covers 3245 cases, and the 53 *it* does not see are this side's alone
...
borch.ts itself has written TS bodies for 2812 cases. **The remaining 486 are two
things**: 340 deliberately not carried across, and 146 owed. The binding
...
alias, so carrying those across would ask the same question twice; the 146 are not
...
And **3298 golden cases** compare all three implementations against **the same
expected values.** The core covers 3245 cases, leaving out the 53 that are
