...
> is that **3428 golden cases pass**, and that is a surface check rather than a
...
print(pipeline(np.zeros((28, 28, 1), dtype=np.uint8)).shape)   # torch.Size([1, 28, 28])
```

**Those ten lines are the example, and until now nothing ran them.** The two
imports stood here alone, and an import that resolves says only that the module
exists — `ToTensor` handing back the wrong axis order would leave both lines
green. `tests/test_document_examples.py` executes this block now and compares the
printed shape, the same way it already does for the autograd example above it:
the picture arrives `(H, W, C)` as torchvision's does and leaves `(C, H, W)`, and
that transposition is the one thing a reader copying these lines depends on.
...
an ES module a browser simply reads (331KB gzipped, 1208KB before compression).
...
It passes **3428 golden cases** — every one in the table but five. Those five are
the core's alone: complex eigenvalues, and there is no complex dtype on this side.
The core covers 3375 cases, and the 53 *it* does not see are this side's alone
...
borch.ts itself has written TS bodies for 3010 cases. **The remaining 418 are two
things**: 351 deliberately not carried across, and 67 owed. The binding
...
And **3428 golden cases** compare all three implementations against **the same
expected values.** The core covers 3375 cases, leaving out the 53 that are
