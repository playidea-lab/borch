...
The lower two **stand on the same kernels.** `borch-webgpu` is a 9,249 line
...
9,249 lines across seven files.
...
> is that **the binding passes 3712 golden cases**, and that is a surface check
...
It passes **3712 golden cases** — every one in the table but five. Those five are
the core's alone: complex eigenvalues, and there is no complex dtype on this side.
The core covers 3664 cases, and the 53 *it* does not see are this side's alone
...
borch.ts itself has written TS bodies for 3234 cases. **The remaining 483 are two
things**: 444 deliberately not carried across, and 39 owed. The binding
...
And **3717 golden cases** compare all three implementations against **the same
expected values.** The core covers 3664 cases, leaving out the 53 that are
