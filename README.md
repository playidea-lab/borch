...
> is that **3257 golden cases pass**, and that is a surface check rather than a
...
an ES module a browser simply reads (267KB gzipped, 981KB before compression).
...
It passes **3257 golden cases** — every one in the table but five. Those five are
the core's alone: complex eigenvalues, and there is no complex dtype on this side.
The core covers 3209 cases, and the 53 *it* does not see are this side's alone
...
borch.ts itself has written TS bodies for 2789 cases. **The remaining 473 are two
things**: 340 deliberately not carried across, and 133 owed. The binding
...
alias, so carrying those across would ask the same question twice; the 133 are not
...
And **3262 golden cases** compare all three implementations against **the same
expected values.** The core covers 3209 cases, leaving out the 53 that are
