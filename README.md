...
900 줄짜리 결속이고, 그 차이(1.5분 대 1.6분)가 Pyodide 를 한 번 지나는 값이다.

> 그 자리에 한동안 **TF.js 판**이 있었다. 같은 이름, 다른 밑바닥, 2,312 줄. 직접 쓴
...
같은 것을 준다. `borch_webgpu` 도 같은 모양이고 900 줄에 다섯 파일이다.
...
> 닿지 않는다. 그쪽에 대해 말할 수 있는 것은 **골든 859건이 지난다**는 것뿐이고,
...
> 옮겨갔고, 아직 다시 안 쟀다. 골든 859 건이 두 바닥에서 같은 값을 냈으므로 크게
...
파이썬 코드를 **borch.ts 위에서** 돌린다. 900 줄짜리 얇은 결속이고, 위의 네 가지
차이(`await init()`·`await item()`·`scope()`·`.call()`)를 전부 감춘다.
...
같은 골든 **859 건 전부**를 지난다 — borch.ts 자신과 같은 수다. 코어(numpy)는 그중
806 건을 보는데, 나머지 53 건은 코어가 일부러 거절하는 것들(1·3 차원 합성곱,
랭크 7·8)이라 안 묻는다.
...
그리고 **골든 859건**이 세 구현을 **같은 기대값**에 대조한다. (코어는 브라우저 전용
케이스를 건너뛰어 806건을 본다 — 코어가 일부러 거절하는 것을 코어에게 물으면 그건
