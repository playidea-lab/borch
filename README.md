...
> is that **3562 golden cases pass**, and that is a surface check rather than a
...
| **`ops`** | **26 of 39 present** — `import borchvision.ops as ops` gives `nms`, `batched_nms`, `box_iou`, `box_area`, `box_convert`, `clip_boxes_to_image`, `masks_to_boxes`, `remove_small_boxes` and the generalised, distance and complete IoUs. They are box geometry with no weights anywhere in them, so unlike the rest of this library **every one is deterministic** and the golden holds all of them. **Five layers joined them, and finding out why they had not is the interesting half.** The 28 absent names carried reasons about what they are *for* — *it needs a model to be a block of*, *a feature map comes from a model* — every one true and none of them about whether the thing can exist. It is the same shape as `as above — a codec`, which was wrong four times before anyone opened the files. Measured this time: the ingredient list. `Conv2dNormActivation` is a convolution, a norm and an activation; `MLP` is linear layers and dropout; `FrozenBatchNorm2d` is an affine transform over four buffers; `SqueezeExcitation` is a pool and two 1×1 convolutions; `Permute` is a method. All of it was in the core the whole time. They are also **the first classes here that subclass the backend's `nn.Module`** — and `use(L)` can change which backend that is after import, so they are built on first access and cached per backend rather than frozen onto whichever library attached first. **And the sentence that replaced the old one overreached too.** It read *what is left needs something that is not here*, and it was wrong about ten of the twenty-two: the four detection losses take two tensors, the four DropBlocks take a feature map and a probability, the two StochasticDepths take a tensor and a coin. `cross_entropy` also *takes a model's predictions* and is here. Removing one over-wide sentence by writing another is the failure the gap table exists to catch, committed twice in a row while catching it — so the third pass was name by name. The **12** still absent need one of two things that is genuinely missing: a feature map, which only a model produces (every `RoI` name, the deformable convolution, the pyramid), or 3-D convolution, which the core declines. The random half of the dropouts cannot be frozen against torchvision — two generators — so what is frozen is the settings where they are the identity, and what they *distribute* is checked in pytest. That test caught its own first claim: **torchvision does not drop `p` either.** At `p=0.5` it drops 0.39, because `gamma` is derived as though blocks never overlapped and the more of them there are the more they land on each other |
...
It passes **3562 golden cases** — every one in the table but five. Those five are
the core's alone: complex eigenvalues, and there is no complex dtype on this side.
The core covers 3509 cases, and the 53 *it* does not see are this side's alone
...
borch.ts itself has written TS bodies for 3121 cases. **The remaining 441 are two
things**: 415 deliberately not carried across, and 26 owed. The binding
...
And **3562 golden cases** compare all three implementations against **the same
expected values.** The core covers 3509 cases, leaving out the 53 that are
...
rasteriser, and for two days every browser golden here ran on it — the same 3509
