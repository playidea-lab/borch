...
1. **`__repr__` 을 진짜와 맞추기** — 학습자가 `print(tensor)` 를 가장 많이 한다
2. **오류 메시지를 진짜와 맞추기** — 모양 불일치 메시지가 다르면 검색해도 안 나온다
3. **RNG 동등성** — 같은 씨앗에서 같은 초기 가중치. 지금은 분포만 같다
4. `nn.RNN` — 순환은 numpy 로 정직하게 구현 가능하다 (느릴 뿐)
5. 고차 미분 — `backward(create_graph=True)`

1·2번이 먼저다. **틀린 값보다 낫다는 원칙의 연장**이다 — 메시지가 다르면 학습자는
검색해서 답을 못 찾고, 그것도 일종의 거짓말이다.
