...
5,022 줄짜리 결속이고, 그 차이(1.5분 대 1.6분)가 Pyodide 를 한 번 지나는 값이다.
...
같은 것을 준다. `borch_webgpu` 도 같은 모양이고 5,022 줄에 여섯 파일이다.
...
> 닿지 않는다. 그쪽에 대해 말할 수 있는 것은 **골든 1887건이 지난다**는 것뿐이고,
...
같은 골든 **1887 건 전부**를 지난다. 코어(numpy)는 그중 1834 건을 보는데, 나머지
...
그리고 **골든 1887건**이 세 구현을 **같은 기대값**에 대조한다. (코어는 브라우저 전용
