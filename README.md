...
> 않는다. 그쪽에 대해 말할 수 있는 것은 **골든 859건이 지난다**는 것뿐이고, 그것은
...
같은 골든 **806 건**을 지난다. 코어(numpy)가 보는 것과 같은 수이고, 자매 전용
...
그래프가 끊긴 것을 못 본다 — 값은 맞기 때문이다. 실제로 자매의 `roll` 과
`masked_select` 가 그렇게 조용히 끊겨 있었고 그때 골든은 전부 초록이었다.

그리고 **골든 859건**이 코어와 자매 라이브러리를 **같은 기대값**에 대조한다.
(코어는 자매 전용 케이스를 건너뛰어 806건을 본다 — 코어가 일부러 거절하는 것을
...
JSON 만으로 코어 792건, 자매 845건이 대조된다.
