...
MODULES = [
    ...
    ("fuse", "fuse",
     {"ko": "캡처된 스텝 위의 융합 — 서로를 먹이는 원소별 커널들이 하나가 된다. `Capture.fuse()`가 부른다.",
      "en": "Fusion over a captured step — elementwise kernels that feed each other become one. Called by `Capture.fuse()`."}),
    ("onnx", "onnx",
     {"ko": "한 번의 forward 를 추적해 ONNX 파일로 쓴다. `torch.onnx.export` 자리.",
      "en": "One forward, traced and written as an ONNX file. Where `torch.onnx.export` would be."}),
    ("compile", "compiled",
     {"ko": "`torch.compiled`·`torch.capture` — 스텝을 한 번 기록해 되감는다. 융합·메모리 계획·측정으로 고른 커널·첫 호출의 비용이 여기 있다.",
      "en": "`torch.compiled` and `torch.capture` — a step recorded once and replayed: fusion, the memory plan, the kernels chosen by measurement, what the first call costs."}),
    ("checkpoint", "checkpoint",
     {"ko": "`torch.utils.checkpoint.checkpoint` — 구간의 출력만 남기고 backward 에서 다시 계산한다.",
      "en": "`torch.utils.checkpoint.checkpoint` — a segment keeps only its output and recomputes in the backward."}),
    ("stream", "stream",
     {"ko": "동결 가중치 창으로 순차 스택을 흘려보낸다 — 블록이 돌기 직전에 두고 직후에 비워, 창보다 큰 모델이 돈다(`docs/SCALE.md`).",
      "en": "A sequential stack streamed through a frozen-weight window — a block's weights placed just before it runs and evicted after, so a model larger than the window runs (`docs/SCALE.md`)."}),
    ("stream_module", "stream · module",
     {"ko": "실제 모듈을 `StreamBlock` 으로 — 허브 백본 같은 모듈이 창을 타게 하는 어댑터.",
      "en": "A real module turned into a `StreamBlock` — the adapter that lets a hub backbone ride the window."}),
    ("stream_train", "stream · train",
     {"ko": "흘려보내는 동결 백본 위에서 어댑터(LoRA)를 학습한다 — forward 와 backward 모두 그 블록이 도는 동안만 가중치가 상주한다.",
      "en": "Training an adapter (LoRA) on a streamed frozen backbone — a block's weights resident only while it runs, in both passes."}),
    ("half", "half",
     {"ko": "호스트 f32 ↔ f16 비트 변환 — 동결 가중치를 창에 절반 바이트로 두기 위한 것. dtype 이 아니다.",
      "en": "Host f32 ↔ f16 bit conversion — frozen weights kept in the window at half the bytes. Not a dtype."}),
    ("quant", "quant",
     {"ko": "호스트 채널별 int8 양자화 — 동결 가중치를 창에 4 분의 1 바이트로 두고, 합성곱의 int8 경로가 읽는 tap-major 팩도 여기서 만든다.",
      "en": "Host per-channel int8 quantisation — frozen weights in the window at a quarter of the bytes, and the tap-major pack the convolution's int8 path reads."}),
    ("cpu", "cpu",
    ...
    ("cpu/load", "cpu · kernels",
     {"ko": "wasm SIMD 커널 모듈을 열고 검증하는 로더. 오프셋 위의 함수 열일곱 개.",
      "en": "The loader that verifies and instantiates the wasm SIMD kernel module — seventeen functions over offsets."}),
    ("cpu/safetensors", "cpu · safetensors",
     {"ko": "장치 없이 체크포인트를 호스트 배열로 읽는다. F32 만.",
      "en": "A checkpoint to host arrays, without a device. F32 only."}),
    ("cpu/graph", "cpu · graph",
     {"ko": "CPU 장치가 실행하는 그래프와 그것을 짓는 빌더. BatchNorm 접기, 가중치 재포장, 채널 패딩이 여기서 일어난다.",
      "en": "The graph the CPU device runs and the builder that makes it — BatchNorm folded, weights repacked, channels padded here."}),
    ("cpu/runner", "cpu · runner",
     {"ko": "그래프의 forward. 크기별 풀로 마지막 사용 시점에 버퍼를 돌려준다.",
      "en": "A graph's forward, with a size-keyed pool returning each buffer at its last use."}),
    ("cpu/train", "cpu · train",
