...
NOT_PORTED = {
    ...
    "dtype::": (172, "아직 — 형 보존은 borch.ts 도 지켜야 한다"),
    ...
    # 42 → 11. 31 건을 옮겼고, 남은 11 은 **이름이 저쪽에 없다.** 난수 넷과
    # `trapezoid` 는 결속이 파이썬에서 numpy 로 만든다 — 그래서 borch.ts 를 TS 로
    # 쓰는 쪽에는 그 이름이 아예 없고, 결속조차 GPU 에서 값을 내렸다 도로 올린다.
    "stat::": (11, "**없음** — `trapezoid`·난수 넷이 저쪽에 없다(결속이 numpy 로 만든다)"),
