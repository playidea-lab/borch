...
NOT_PORTED = {
    ...
    # **0 → 5.** `linalg::` 줄은 열일곱을 다 옮기면서 지웠는데 `eig` 로 다시 열렸다.
    # 대칭용 `eigh` 는 borch.ts 에 있지만 **비대칭 고유분해는 없다** — 헤센베르크
    # 변환 + QR 반복이라 다른 크기의 일이다. 코어에만 두었고 케이스도
    # `CORE_ONLY_PREFIXES` 라 결속도 안 본다. **"아직" 이 아니라 "없음" 이다.**
    "linalg::": (5, "**없음** — borch.ts 에 비대칭 고유분해가 없다"),
    # **borch.ts 에 결함이 있어 잠시 여기 있다.** 열넷은 저쪽에도 조립해 두었고
    # 값이 맞는데, 밑의 1 차원 `fft` 가 **복소수 입력을 마지막이 아닌 축으로**
    # 변환할 때 틀린다(실측 — 실수 입력은 두 축 다 맞고 복소수도 `dim=-1` 은 맞다).
    # 기존 `fft` 케이스가 전부 1 차원 벡터라 그 조합을 한 번도 안 물었다.
    # 저쪽이 고쳐지면 이 줄을 지우고 `CORE_ONLY_PREFIXES` 에서도 뺀다.
    "fft::": (21, "**결함** — borch.ts 의 복소수 fft 가 축 0 에서 틀린다"),
    "dtype::": (158, "아직 — 형 보존은 borch.ts 도 지켜야 한다"),
