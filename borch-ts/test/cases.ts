...
function addNdim(out: Map<string, Case>, inp: Inputs): void {
  ...
  // **matmul 이 배치와 브로드캐스트를 하는지.** 이 표의 matmul 케이스는 전부 2 차원
  // 이었고, 2 차원에서 `matmul` 과 `mm` 은 같은 함수다 — 그래서 이쪽은 전부 `mm` 으로
  // 답해 왔고 **배치 경로가 아예 없다는 사실이 한 번도 드러나지 않았다.**
  //
  // 값은 Python 쪽 `tests/cases.py` 와 **같은 수를 다시 적은 것**이다. 값이 갈리면
  // 대조가 대조가 아니게 되므로 그대로 옮겼다.
