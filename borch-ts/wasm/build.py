...
def parse_kernels_ts(text):
    ...
    return {
        ...
        "shared_sha256": one("KERNELS_SHARED_WASM_SHA256"),
        "shared": blob("KERNELS_SHARED_WASM_BASE64"),
    }


# The shared module: the same relaxed kernels, linked over an *imported, shared* memory so
# that every worker of a pool instantiates it over one buffer (`src/cpu/threads.ts`).
# `--export=__stack_pointer` lets each worker be given its own shadow stack — Rust keeps
# that stack in linear memory, at the same address in every instance, and without the
# export the workers spill registers over each other's frames (measured: results off by
# 1e-5 and different on every run). `--max-memory` is the most an importer may declare.
SHARED_FEATURES = ",+relaxed-simd,+atomics,+bulk-memory,+mutable-globals"
SHARED_LINK = ("--import-memory", "--shared-memory", "--max-memory=4294967296", "--export=__stack_pointer")
SHARED_IMPORTS = 1  # env.memory, and nothing else


def _cargo(cargo, features, extra_args, link_args=(), imports=0):
    flags = "-C target-feature=+simd128" + features + "".join(f" -C link-arg={a}" for a in link_args)
    ...
    if wasm_import_count(data) != imports:
        sys.exit(f"the module imports {wasm_import_count(data)} things, expected {imports} — the kernels are meant to need nothing but, for the shared build, the memory")
    return data


def build():
    """The three modules: `(strict, relaxed, shared, rustc)`."""
    ...
    shared = _cargo(cargo, SHARED_FEATURES, ["--features", "relaxed"], SHARED_LINK, SHARED_IMPORTS)
    rustc = subprocess.run(["rustc", "--version"], capture_output=True, text=True, check=True).stdout.strip()
    if not (exports_of(strict) == exports_of(relaxed) == exports_of(shared)):
        sys.exit("the three modules export different names — they are meant to be one interface")
    if "__stack_pointer" not in dict(wasm_exports(shared)):
        sys.exit("the shared module does not export __stack_pointer — the workers could not be given their own stacks")
    return strict, relaxed, shared, rustc
...
def render(strict, relaxed, shared, rustc):
 ...
 *
 * Three modules from one source. The strict one ({len(strict)} bytes) rounds every multiply and
 ...
 * ({len(shared)} bytes) is the relaxed kernels linked over an imported, shared memory, for a
...
{_blob(relaxed)};
export const KERNELS_SHARED_WASM_SHA256 = "{hashlib.sha256(shared).hexdigest()}";
export const KERNELS_SHARED_WASM_BYTES = {len(shared)};
...
def main(argv):
    strict, relaxed, shared, rustc = build()
    text = render(strict, relaxed, shared, rustc)
    if "--check" in argv:
        if not OUT.exists():
            sys.exit(f"{OUT.relative_to(ROOT)} does not exist — run without --check")
        committed = parse_kernels_ts(OUT.read_text(encoding="utf-8"))
        if committed["wasm"] != strict or committed["relaxed"] != relaxed or committed["shared"] != shared:
            sys.exit(f"rebuilt bytes differ from {OUT.relative_to(ROOT)} (committed {committed['rustc']}, now {rustc}) — run npm run build:wasm")
        print(f"{OUT.relative_to(ROOT)}: strict {len(strict)} + relaxed {len(relaxed)} + shared {len(shared)} bytes, match the rebuild")
        return 0
    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(text, encoding="utf-8")
    print(f"{OUT.relative_to(ROOT)}: strict {len(strict)} + relaxed {len(relaxed)} + shared {len(shared)} bytes, exports {', '.join(exports_of(strict))}")
    return 0
