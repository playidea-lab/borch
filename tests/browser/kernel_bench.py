...
def main(argv):
    ...
    sweep = next((a.split("=", 1)[1] for a in argv if a.startswith("--sweep=")), "")
    reps = next((a.split("=", 1)[1] for a in argv if a.startswith("--reps=")), "")
    if refuse_if_screen_off("the kernel bench"):
        return 1
    port, shutdown = serve(ROOT)
    url = f"http://127.0.0.1:{port}/tests/browser/kernel_bench.html?bench={bench}" + (f"&shapes={shapes}" if shapes else "") + (f"&sweep={sweep}" if sweep else "") + (f"&reps={reps}" if reps else "")
