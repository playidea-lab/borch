...
in a worker, on this tab's GPU.
    ...
    uv run --with playwright python tests/browser/profile_py.py [--build] [--batch=16] [--size=96] [--steps=5] [--headless]
...
def main(argv):
    ...
    url = f"http://127.0.0.1:{port}/tests/browser/profile_py.html?wheel=/{wheel}&batch={batch}&size={size}&model={model}&steps={steps}"
