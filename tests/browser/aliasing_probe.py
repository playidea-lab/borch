"""Views and aliasing — the patterns a golden of single ops misses.

    npm run build:ts && npm run wheel:py
    uv run --with playwright python tests/browser/aliasing_probe.py [--build]

Opens aliasing_probe.html: several views of one base flowing through a chain and
summing their gradients back (the qkv split's shape), each checked against a numpy
reference and once through torch.compiled(check=True). Passes today because the ops
copy; it is the net that catches narrow/select wired as a view but wrong.
"""
import glob
import os
import pathlib
...
def main(argv):
    ...
    url = f"http://127.0.0.1:{port}/tests/browser/aliasing_probe.html?wheel=/{wheel}&batch={batch}&size={size}"
    ...
    ok = ("faults 0" in done and "ERR" not in done and bool(worst)
          and float(worst.group(1)) <= 1e-4)
    print("**views and aliasing hold — grads match the reference, capture is clean**" if ok
          else "**an aliasing pattern is wrong** — see above")
