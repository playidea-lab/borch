...
CEILINGS = {
    ...
    # 3192 → 3194. `dataset::IDX labels(short by two)=거절` 의 `거절` 두 자다. **케이스
    # 이름은 양쪽이 한 글자도 다르면 안 되므로** 이 두 자는 이 파일이 막으려는 종류의
    # 한국어가 아니다 — 새로 쓴 산문이 아니라 골든 키다. 천장이 이런 이유로 오르는 것은
    # 괜찮고, 주석 없이 오르는 것이 안 괜찮다.
    # 케이스 이름은 파이썬 쪽과 **글자까지 같아야** 하는 키라, 이쪽만 영어로 쓰면
    # 두 표가 서로 다른 케이스를 가리키게 된다. 번역 대상이 아니라 대조 대상이다.
    # 3194 → 3196. `act::nn.<이름>(inplace)/같은 객체` 여섯 줄의 `같은 객체` 인데, 여섯
    # 개가 한 템플릿 리터럴에서 나오므로 소스에는 한 번만 적힌다. 앞 문단과 같은 종류다 —
    # 골든 키이지 새로 쓴 산문이 아니다.
    # 3196 → 3221. `pool::nn.AvgPool*(테두리 채움|가장자리 빼기|나눗수 지정|올림)` 과
    # `LPPool*(올림)` 의 이름들이다. 앞의 두 문단과 같은 이유로 오른다 — 케이스 이름은
    # 골든 키이고 한 글자도 다르면 그 케이스가 조용히 안 돈다. **이 천장이 막는 것은
    # 새로 쓰는 한국어 산문이지 이미 얼어 있는 키가 아니다**, 그리고 그 구분을 매번
    # 적어 두는 것이 천장이 슬금슬금 오르는 것과 다른 점이다.
    # 3221 → 3247. `misc::층::Embedding*` 열한 줄의 이름이다 — `표가 줄었다`,
    # `색인 안 된 행`, `새 표와 준 표`, `전부`. 같은 이유이므로 같은 처리를 한다.
    # 3247 → 3335. `unpool::` 스무 줄의 이름이다 — `이름이 둘인 같은 계산`,
    # `층::repr::…`, `분수::표본 없이(모양과 범위)`, `적응형softmax::…` 넷, 그리고
    # 그 케이스가 답으로 내는 문자열 `(1, 1, 3, 3) 안에 있는 것=9`. **마지막 것은 이름이
    # 아니라 값이고**, 골든이 그 문자열을 그대로 얼려 두었으므로 한 글자도 못 바꾼다.
    # 3335 → 3339. `run.py` 의 원장 주석이 케이스 이름 `1차원 표적` 을 인용한 넉 자다.
    # **이것은 케이스 이름이 아니라 산문 속의 인용이고**, 그래서 앞의 것들과 종류가
    # 다르다 — 다만 무엇을 가리키는지 말하려면 그 이름을 그대로 써야 하고, 영어로
    # 옮겨 적으면 독자가 원장의 그 줄을 찾지 못한다.
    "borch-ts/test": 3339,
}
