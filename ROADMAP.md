...
browsertorch 의 원칙은 **"없는 기능이 틀린 답보다 낫다"** 이다.
...
오히려 browsertorch 가 빠르다.

| | torch (CPU) | browsertorch (네이티브) | browsertorch (브라우저) |
|---|---|---|---|
...
브라우저에서 진짜로 학습을 돌려야 한다면, 답은 browsertorch 가 아니라 원격 장비다.
...
- **결정**
  ...
  코어 `browsertorch` 는 **"MNIST 급까지"와 순수 파이썬 17KB 휠을 그대로 유지한다.**
  천장을 올리는 일은 **별도 배포판**(`browsertorch-webgpu`)이 맡는다.
